...
# Identifies the agent from cwd via ~/.agent-team-os/AGENT_MAP.json.
...
if [[ ! -f "$HOME/.agent-team-os/AGENT_MAP.json" ]]; then
    mkdir -p "$HOME/.agent-team-os/"{inboxes,registry,threads,outbox,locks,personas}
    SRC_EXAMPLE=""
    ...
    if [[ -n "$SRC_EXAMPLE" ]]; then
        cp "$SRC_EXAMPLE" "$HOME/.agent-team-os/AGENT_MAP.json"
